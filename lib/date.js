...
Date.prototype.toFormat = function (spec) {
  var result = [], i = 0,
      ...
      jan1 = new Date(value.getFullYear(), 0, 1);
    ...
    if (prev === '%') {
      ...
      case 'U':
        result[result.length] = T([
          Math.ceil((((value - jan1) / 86400000) + jan1.getDay() + 1) / 7)]);
        break;
      case 'W':
        // Monday as the first day of the week
        var jan1Day = (jan1.getDay() + 6) % 7,
            week = Math.ceil((((value - jan1) / 86400000) + jan1Day + 1) / 7);

        if (jan1Day > 3) week -= 1;
        result[result.length] = T([week]);
        break;
      ...
      case 'h':
        ...
        break;
      case 'e':
        result[result.length] = fmt("{: >2}", value.getDate());
        break;
      ...
      case 'A':
        ...
        break;
      case 'u':
        result[result.length] = (value.getDay() + 6) % 7 + 1;
        break;
      case 'w':
      ...
      case 'c':
        ...
        break;
      case 'x':
        result[result.length] = value.toLocaleDateString();
        break;
      case 'X':
      ...
      case 'D':
        ...
        break;
      case 'F':
        result[result.length] = fmt("{:%Y-%m-%d}", value);
        break;
      case 'R':
        result[result.length] = fmt("{:%H:%M}", value);
        break;
      case 'T':
        result[result.length] = fmt("{:%H:%M:%S}", value);
        break;
      case 'p':
        result[result.length] = value.getHours() > 11 ? "PM" : "AM";
        break;
      default:
        result[result.length] = chr;
      }
    } else if (chr !== '%') {
      result[result.length] = chr;
    }
